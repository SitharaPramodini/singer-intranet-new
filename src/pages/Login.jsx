...
const Login = () => {
    ...
    // Use formData state directly here
        ...
        if (response.ok) {
            ...
            localStorage.setItem(
                ...
                JSON.stringify({
                    ...
                    role: data.user.role, // save role in localStorage too
                })
            );
                ...
                <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl border border-white/20 p-8 transform transition-all duration-700 hover:scale-[1.02] animate-fade-in">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="relative group">
                            <label
                                htmlFor="username"
                                className={`absolute left-4 transition-all duration-300 pointer-events-none z-50 ${focusedField === 'username' || formData.username
                            ...
                            <input
                                ...
                                disabled={isLoading}
                                className={`w-full px-4 py-4 border-2 rounded-xl bg-white/50 backdrop-blur-sm transition-all duration-300 outline-none ${focusedField === 'username'
                        ...
                        <div className="relative group">
                            <label
                                htmlFor="password"
                                className={`absolute left-4 transition-all duration-300 pointer-events-none z-50 ${focusedField === 'password' || formData.password
                                        ? '-top-2 text-xs bg-white px-2 text-[#ed1b24] font-medium'
                                        : 'top-4 text-gray-500'
                                    }`}
                            >
                            ...
                            <input
                                ...
                                onKeyPress={handleKeyPress}
                                disabled={isLoading}
                                className={`w-full px-4 py-4 border-2 rounded-xl bg-white/50 backdrop-blur-sm transition-all duration-300 outline-none ${focusedField === 'password'
                                        ? 'border-[#ed1b24] shadow-lg shadow-red-100 transform -translate-y-1'
                                        : 'border-gray-200 hover:border-gray-300'
                                    } ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
                                required
                        ...
                        <button
                            ...
                            className={`w-full py-4 px-6 bg-gradient-to-r from-[#ed1b24] to-red-700 text-white font-semibold rounded-xl transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl focus:outline-none focus:ring-4 focus:ring-red-200 relative overflow-hidden group ${isLoading ? 'opacity-60 cursor-not-allowed' : 'hover:shadow-2xl shadow-lg'
                                }`}
                            type="button"
                        >
